...
async def test_navigator_path_building():
    ...
    mock_neo4j.find_step_free_path.return_value = [
        {"name": "Zone_1", "type": "Zone", "distance_m": 50, "step_free": True, "estimated_time_s": 60},
        {"name": "Zone_2", "type": "Zone"}
    ]
    
    agent = NavigatorAgent(llm_router=mock_llm, neo4j=mock_neo4j)
    
    req = NavigationRequest(start_location="Zone_1", destination_intent="Section 101", step_free=True, language="en")
