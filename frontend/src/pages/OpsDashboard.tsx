...
export function OpsDashboard() {
  ...
  const [tab, setTab] = useState<'overview' | 'actions' | 'sustainability' | 'audit' | 'efficiency'>('overview')
  ...
  const sorted = useMemo(() => [...pending].sort(
    ...
    <div className="ops-app">
      <header className="ops-header">
        <div>
          ...
          <h1>Ops <span className="text-accent">Dashboard</span></h1>
        </div>
        <div className="ops-header-meta">
          {user && (
            <span className="role-badge" aria-label={`Signed in as ${user.role}`}>
          ...
          <nav className="ops-tabs" aria-label="Dashboard sections">
            {(['overview', 'actions', 'sustainability', 'efficiency', 'audit'] as const)
              .filter((t) => !(isVolunteer && (t === 'audit' || t === 'efficiency')))
              .map((t) => (
              <button
                key={t}
      ...
      {error && (
        <div className="bg-red-900 text-white p-3 text-center" role="alert" aria-live="assertive">
          {error}
        </div>
      )}
      ...
      <main className="ops-main">
        {isVolunteer && tab === 'overview' && sorted.length > 0 && (
          <section className="panel volunteer-banner" aria-labelledby="volunteer-title">
            ...
            <section className="panel" aria-labelledby="actions-title">
              ...
              <div className="action-list compact">
                {sorted.slice(0, 3).map((action) => (
                  <article key={action.id} className={`action ${action.priority}`}>
                    ...
                    <div className="action-meta">
                      <span>
                        <Users size={14} /> {action.affected_population.toLocaleString()} fans
                      </span>
                      <span>
                        <Clock size={14} /> {action.time_to_impact_min ?? 'N/A'} min
                      </span>
                    </div>
                  </article>
                ))}
              ...
              {sorted.map((action) => (
                <article key={action.id} className={`action ${action.priority}`}>
                  ...
                  <div className="action-meta">
                    ...
                    <span>
                      <Clock size={14} /> {action.time_to_impact_min ?? 'N/A'} min impact
                    </span>
                  ...
                  <div className="action-buttons">
                    {isOrganizer ? (
                      <>
                        <button className="btn-success" onClick={() => approve(action.id)}>
                          <Check size={16} /> Approve
                        </button>
                        <button className="btn-danger" onClick={() => reject(action.id)}>
                          <X size={16} /> Reject
                        </button>
                      </>
                    ) : (
                      <p className="muted">Volunteer view — read-only. Contact organizer to approve.</p>
                    )}
                  </div>
                </article>
              ))}
              {sorted.length === 0 && <p className="muted">All clear — no pending actions.</p>}
            </div>
          </section>
        )}
            ...
            <div className="sustain-grid">
              ...
              <div className="sustain-card">
                ...
                <p className="big-number">
                  {sustain.estimated_co2_kg_per_1000_fans ?? sustain.estimated_co2_kg ?? '--'} kg
                </p>
          ...
          <section className="panel" aria-labelledby="eff-title">
            ...
            <div className="efficiency-grid">
              ...
              <div className="sustain-card">
                ...
                <p className="muted">
                  {efficiency.cache_hits} hits · {efficiency.cache_misses} misses
                </p>
              </div>
              <div className="sustain-card">
                <h4>Calls</h4>
              ...
              <div className="sustain-card">
                ...
                <p className="muted">{efficiency.estimated_tokens_saved} tokens saved via cache</p>
              </div>
            </div>
          </section>
        )}
                ...
                {auditLogs.length === 0 && (
                  <tr>
                    <td colSpan={5} className="muted center">
                      No audit entries yet. Approve or reject an action to populate the log.
                    </td>
                  </tr>
                )}
                {auditLogs.map((log) => (
