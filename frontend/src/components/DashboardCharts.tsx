...
const generateHistoricalData = (zones: Zone[]) => {
  const currentAvg = zones.length ? zones.reduce((acc, z) => acc + z.density, 0) / zones.length : 0.5
  const data = []
  const now = new Date()
        ...
        <div style={{ width: '100%', height: '250px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={historyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              />
              <Line type="monotone" dataKey="density" stroke="#38bdf8" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <section className="panel" aria-labelledby="chart-staff-title">
        <div className="panel-head">
          <div>
            ...
            <h2 id="chart-staff-title">Active Response Teams</h2>
          </div>
        </div>
        <div style={{ width: '100%', height: '250px' }}>
          <ResponsiveContainer width="100%" height="100%">
